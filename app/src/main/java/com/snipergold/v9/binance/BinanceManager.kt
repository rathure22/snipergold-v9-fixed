...
object BinanceManager {
    ...
    suspend fun getBalanceParsed(context: Context): Pair<String, String> = withContext(Dispatchers.IO) {
        val keys = SecureApiStorage.loadKeys(context)
        val apiKey = keys.second
        val secret = keys.third
    ...
    suspend fun placeRealOrder(context: Context, side: String, lot: Double, entryPrice: Double = 4376.36): OrderResult = withContext(Dispatchers.IO) {
        val keys = SecureApiStorage.loadKeys(context)
        val apiKey = keys.second
        val secret = keys.third
        ...
        val symbol = "PAXGUSDT"
        val timestamp = System.currentTimeMillis()
        val query = "symbol=$symbol&side=$side&type=MARKET&quantity=$lot&timestamp=$timestamp"
        val signature = hmacSHA256(query, secret)
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                OrderResult(true, orderId, "REAL $side $lot Lot - OrderID: $orderId", filledPrice)
            } else {
                val json = JSONObject(body)
